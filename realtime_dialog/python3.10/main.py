...
async def main() -> None:
    ...
    parser.add_argument("--mic-source", type=str, default="pyaudio", choices=["pyaudio", "ros2"],
                        help="麦克风输入源: pyaudio=本地直采(默认); ros2=订阅 /avvtn/mic_pcm 拿 AVVTN AEC 后的干净 PCM")

    args = parser.parse_args()

    session = DialogSession(ws_config=config.ws_connect_config, output_audio_format=args.format, audio_file_path=args.audio,
                            mod=args.mod, recv_timeout=args.recv_timeout, mic_source=args.mic_source)
    await session.start()
